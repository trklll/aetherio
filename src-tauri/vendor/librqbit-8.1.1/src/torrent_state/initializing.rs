...
impl TorrentStateInitializing {
    ...
    pub async fn check(&self) -> anyhow::Result<TorrentStatePaused> {
        ...
        let have_pieces = match have_pieces {
            Some(h) => h,
            None => {
                // Aetherio patch (streaming mode): skip the full checksum validation of
                // pre-existing files. The full check hashes every byte of every file in the
                // torrent and takes minutes for large batches (e.g. 100GB+ BD releases),
                // while torrents only become playable after it completes. For a streaming
                // cache this is not worth it:
                //   - freshly downloaded pieces are still verified against the torrent
                //     hashes when the swarm delivers them,
                //   - resume from existing partial data is still handled cheaply by
                //     `validate_fastresume` above whenever a bitfield is available.
                // Without a bitfield we simply assume nothing on disk is verified, so the
                // torrent goes live immediately and only missing pieces are fetched.
                info!("no fastresume bitfield, skipping initial checksum validation (streaming mode)");
                let empty_have_pieces =
                    BF::from_boxed_slice(vec![0u8; self.metadata.lengths.piece_bitfield_bytes()].into_boxed_slice());
                if let Err(e) = bitv_factory
                    .store_initial_check(id, empty_have_pieces.clone())
                    .await
                {
                    warn!(error=?e, "error storing empty initial check bitfield");
                }
                empty_have_pieces.into_dyn()
            }
        };
