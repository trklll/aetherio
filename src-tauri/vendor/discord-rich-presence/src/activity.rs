...
/// The kind of a Discord activity button.
#[derive(Serialize_repr, Clone, Copy, PartialEq)]
#[repr(u8)]
pub enum ButtonType {
    /// A link button that opens a URL when clicked.
    Link = 0,
    /// A custom-id button that fires an `ACTIVITY_JOIN` event when clicked.
    CustomId = 1,
}
...
impl<'a> Button<'a> {
    ...
    /// Creates a new custom-id `Button` with the given
    /// label and `custom_id`.
    ///
    /// Clicking this button fires an `ACTIVITY_JOIN` event
    /// on the local IPC client, carrying the `custom_id` in
    /// the payload's `secret` field.
    ///
    /// The label must be 1-32 characters long
    ///
    /// The `custom_id` must be 1-64 characters long
    pub fn new_custom(label: &'a str, custom_id: &'a str) -> Self {
        Button {
            label,
            url: None,
            custom_id: Some(custom_id),
            button_type: ButtonType::CustomId,
        }
    }
}
