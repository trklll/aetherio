  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!tmdbId) {
          setError("No se encontr� este t�tulo en TMDB.");
          return;
        }
        ...
        if (!cancelled) setError("No se pudo cargar la secci�n.");
      } finally {
...
async function mapAnimeCharacters(id: string, tmdbData: any, tmdbId: number): Promise<PersonItem[]> {
  ...
  let parsedMal: number | undefined;
  if (lower.startsWith("mal:")) {
    const mal = Number(id.split(":")[1]);
    if (Number.isFinite(mal) && mal > 0) parsedMal = mal;
  }
...
function translateJob(value: string) {
  if (value === "Executive Producer") return "Producci�n ejecutiva";
  if (value === "Producer") return "Producci�n";
  if (value === "Director") return "Direcci�n";
  if (value === "Writer") return "Guion";
  if (value === "Creator") return "Creaci�n";
  return value;
}
